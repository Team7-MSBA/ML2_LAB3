...
mydata<-read.csv("Lab3Data.csv",sep=",",header=T)
discardcols <- names(mydata) %in% c("customerID")
mydata<- mydata[!discardcols]
mydata<-na.omit(mydata)

#build the train and test
set.seed(2016)
train<-sample(1:nrow(mydata), size=5042)
...
mymodel<-rpart(Churn~.,data=mydata[train,],method="class",
...
besttable<-table(mydata.test$Churn, mymodel.prune.predict,dnn=c("Actual", "Predicted"))
besterror=(besttable[2]+besttable[3])/nrow(mydata.test)
besttype1=besttable[3]/(besttable[1]+besttable[3])
besttype2=besttable[2]/(besttable[2]+besttable[4])

####Other Attempts####
#build the tree
tree.mydata=tree(Churn~.,data = mydata[train,])
mymodel <- rpart(Churn ~ .,data=mydata[train,],method="class",
...
mymodel.predict <- predict(mymodel, newdata=mydata.test, type="class")
...
###Predict on the test set
mymodel.prune.predict <- predict(mymodel.prune, newdata=mydata.test, type="class")
mytable2<-table(mydata.test$Churn, mymodel.prune.predict,dnn=c("Actual", "Predicted"))
...
mymodel.predict <- predict(mymodel, newdata=mydata.test, type="class")
mytable<-table(mydata.test$Churn, mymodel.predict,dnn=c("Actual", "Predicted"))
errorRate[3]=(mytable[2]+mytable[3])/nrow(mydata.test)
...
####Loss Matrix####
mymodel.prune<-rpart(Churn~.,data=mydata[train,],method="class",
                     parms=list(split="information"),control=rpart.control(usesurrogate=0,
...
###Predict on the test set
mymodel.prune.predict <- predict(mymodel.prune, newdata=mydata.test, type="class")
mytable=table(mydata.test$Churn, mymodel.prune.predict,dnn=c("Actual", "Predicted"))
errorRate[4]=(mytable[2]+mytable[3])/nrow(mydata.test)
...
for (j in loss){
  for(i in loss){
    ...
    mymodel.prune.predict <- predict(mymodel.prune, newdata=mydata.test, type="class")
    mytable=table(mydata.test$Churn, mymodel.prune.predict,dnn=c("Actual", "Predicted"))
    output[j,i]=(mytable[2]+mytable[3])/nrow(mydata.test)
